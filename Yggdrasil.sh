...
extract_data() {
    service_name=$1
    destination_path="extracted_data/$service_name"

    if [ ! -d "$destination_path" ]; then
        echo "Creation of extract destination folder : $destination_path"
        mkdir -p "$destination_path"
    fi

    docker-compose run --rm -v "$destination_path:/host_destination" $service_name cp -R /container_data /host_destination
    echo "Data has been extracted here : $destination_path"
}
